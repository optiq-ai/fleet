...
interface FuelQualityTestProps {
  transactions: any[];
...
const TestCardHeader = styled.div<{ status: 'pass' | 'warning' | 'fail' }>`
  padding: 12px 16px;
...
const ResultValue = styled.td<{ status: 'pass' | 'warning' | 'fail' }>`
  color: ${props => 
    props.status === 'pass' ? '#2e7d32' : 
    props.status === 'warning' ? '#f57f17' : 
    '#c62828'
...
const StatCard = styled.div<{ status: 'pass' | 'warning' | 'fail' }>`
  padding: 16px;
  background-color: ${props => 
    props.status === 'pass' ? '#e8f5e9' : 
    props.status === 'warning' ? '#fff8e1' : 
...
const FuelQualityTest: React.FC<FuelQualityTestProps> = ({ 
  transactions,
  onTestResults
}) => {
  ...
  useEffect(() => {
    ...
    const generateTestResults = () => {
      ...
      const results = filteredTransactions.map(transaction => {
        ...
        const generateTestValue = (min: number, max: number) => {
        ...
        const getOverallStatus = () => {
          ...
          if (cetaneNumber !== null) {
            statuses.push(getCetaneNumberStatus() as 'pass' | 'warning' | 'fail');
          }
  ...
  const renderTestCard = (result: any) => {
    return (
